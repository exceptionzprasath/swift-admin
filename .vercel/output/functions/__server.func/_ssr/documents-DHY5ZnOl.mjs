...
import { i as inr, n as computePayroll } from "./store-Dj1aT4sf.mjs";
import { t as require_jspdf_node_min } from "../_libs/jspdf.mjs";
import { t as autoTable } from "../_libs/jspdf-autotable.mjs";
import { t as require_lib } from "../_libs/jszip+[...].mjs";
import { a as Header, c as PageNumber, d as Table, f as TableCell, h as WidthType, i as Footer, l as Paragraph, m as TextRun, n as BorderStyle, o as HeadingLevel, p as TableRow, r as File, s as Packer, t as AlignmentType, u as ShadingType } from "../_libs/docx.mjs";
import { t as require_FileSaver_min } from "../_libs/file-saver.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/documents-DHY5ZnOl.js
...
var DEFAULT_TEMPLATES = [
	{
		...
		title: "Offer Letter",
		category: "Onboarding",
		description: "Pre-joining offer with CTC breakup.",
		body: `Dear {{name}},
...
{{company}}`
	},
	{
		...
		category: "Onboarding",
		description: "Formal appointment with terms & salary breakup.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Onboarding",
		description: "First-day joining acknowledgement.",
...
Signature of Employee: __________________     Signature of HR: __________________`
	},
	{
		...
		category: "Onboarding",
		description: "Internship engagement letter.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Onboarding",
		description: "Confidentiality undertaking.",
...
Signature: __________________`
	},
	{
		...
		category: "Onboarding",
		description: "Full contract with terms.",
...
For {{company}}                                             Employee`
	...
	{
		key: "probation_extension",
		title: "Probation Extension",
		category: "Confirmation",
		description: "Extends probation period.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Confirmation",
		description: "Confirms employment post-probation.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Movement",
		description: "Promotion with new designation.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Movement",
		description: "Inter-branch / inter-department transfer.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Discipline",
		description: "Notice to explain conduct.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Discipline",
		description: "Suspension pending enquiry.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Exit",
		description: "Relieving on resignation acceptance.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		description: "Service certificate on exit.",
...
For {{company}}`
	},
	{
		...
		category: "Exit",
		description: "F&F statement summary.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		category: "Exit",
		description: "Departmental clearance form.",
...
Signatures: __________________`
	},
	{
		...
		category: "Verification",
		description: "Salary certification for banks/visa.",
		body: `TO WHOMSOEVER IT MAY CONCERN
...
For {{company}}`
	},
	{
		...
		category: "Verification",
		description: "Certificate of bonafide employment.",
		body: `TO WHOMSOEVER IT MAY CONCERN
...
For {{company}}`
	},
	{
		...
		category: "Verification",
		description: "Employer-issued address confirmation.",
		body: `TO WHOMSOEVER IT MAY CONCERN
...
For {{company}}`
	...
	{
		key: "notice_common",
		title: "Common Notice / Circular",
		category: "Compliance",
		description: "Notice for all employees.",
		body: `NOTICE
...
HR Department`
	...
	{
		key: "memo_late_attendance",
		title: "Memo — Late Attendance",
		category: "Discipline",
		description: "Late attendance memo.",
		body: `Dear {{name}},
...
For {{company}}`
	},
	{
		...
		title: "Custom Letter",
		category: "Custom",
		description: "Blank template — write your own.",
		body: `Dear {{name}},
...
function buildVars(company, employee) {
	...
	return {
		...
		gstin: company.gstin,
		today: (/* @__PURE__ */ new Date()).toLocaleDateString("en-IN", {
			day: "2-digit",
			month: "long",
			year: "numeric"
		})
...
var A4_W = 210, A4_H = 297, ML = 14, MR = 14;
var RIGHT = A4_W - MR;
var CONTENT_W = A4_W - ML - MR;
var BOTTOM_SAFE = A4_H - 20;
function pdfHeader(doc, company, title) {
...
function pdfFooter(doc, refId) {
	const pages = doc.getNumberOfPages();
	for (let i = 1; i <= pages; i++) {
		doc.setPage(i);
		doc.setFontSize(7);
		doc.setTextColor(140, 140, 140);
		doc.text(`Generated by SWIFT AI HRMS · ${(/* @__PURE__ */ new Date()).toLocaleString("en-IN")} · Ref: ${refId}`, ML, 289);
		doc.text(`Page ${i} of ${pages}`, RIGHT, 289, { align: "right" });
		doc.setTextColor(0, 0, 0);
	}
}
function generateLetterPDF(company, employee, template, assets) {
	...
	if (assets?.logoDataUrl) try {
		doc.addImage(assets.logoDataUrl, "PNG", RIGHT - 26, 4, 22, 22);
	} catch {}
	if (assets?.watermarkDataUrl) try {
		const gs = doc;
		if (gs.GState && gs.setGState) gs.setGState(new gs.GState({ opacity: .08 }));
	...
	if ([
		...
		autoTable(doc, {
			...
			body: [
				[
					"Basic",
					inr(p.earnings.basic),
					inr(p.earnings.basic * 12)
				],
				[
					"HRA",
					inr(p.earnings.hra),
					inr(p.earnings.hra * 12)
				],
				[
					"Special Allowance",
					inr(p.earnings.special),
					inr(p.earnings.special * 12)
				],
				[
					"Medical",
					inr(p.earnings.medical),
					inr(p.earnings.medical * 12)
				],
				[
					"Conveyance",
					inr(p.earnings.conveyance),
					inr(p.earnings.conveyance * 12)
				],
				[
					"Washing",
					inr(p.earnings.washing),
					inr(p.earnings.washing * 12)
				],
				[
					"Other Allowance",
					inr(p.earnings.other),
					inr(p.earnings.other * 12)
				],
				[
					{
						content: "Gross",
					...
					{
						content: inr(p.gross * 12),
						styles: { fontStyle: "bold" }
					}
				],
				[
					"Employer PF + ESI + Gratuity",
					inr(p.totalEmployer),
					inr(p.totalEmployer * 12)
				],
				[
					{
						content: "Total CTC",
	...
	const sigImg = assets?.authorisedSignatoryDataUrl ?? assets?.hrSignatureDataUrl ?? assets?.mdSignatureDataUrl;
	if (sigImg) try {
		doc.addImage(sigImg, "PNG", ML, y + 4, 55, 18);
	} catch {}
	...
	if (seal) try {
		doc.addImage(seal, "PNG", (A4_W - 30) / 2, y + 2, 30, 30);
	} catch {}
	...
	if (assets?.qrCodeDataUrl) try {
		doc.addImage(assets.qrCodeDataUrl, "PNG", RIGHT - 18, y + 34, 18, 18);
	} catch {}
	pdfFooter(doc, refId);
...
var BRAND_HEX = "14A0AA";
function docxHeader(company, title) {
	return new Header({ children: [new Paragraph({
		...
		children: [new TextRun({
			text: company.legalName + " · " + company.address,
			size: 16,
			color: "6B7280",
			font: "Arial"
		})],
...
function docxFooter() {
	return new Footer({ children: [new Paragraph({
		...
		children: [new TextRun({
			text: "Generated by SWIFT AI HRMS · ",
			size: 14,
			color: "9CA3AF",
			font: "Arial"
		}), new TextRun({
...
async function generateLetterDOCX(company, employee, template) {
	const vars = buildVars(company, employee);
	const paragraphs = renderTemplate(template.body, vars).split(/\n/).map((line) => new Paragraph({
		spacing: { after: 160 },
		children: [new TextRun({
			text: line || " ",
			size: 22,
	...
	const meta = [new Paragraph({
		spacing: { after: 120 },
		children: [new TextRun({
			text: `Date: ${vars.today}`,
			size: 20,
			color: "6B7280",
			font: "Arial"
		}), new TextRun({
			text: `\t\tRef: SWIFT/${template.key.toUpperCase()}/${employee.empCode}`,
			size: 20,
			color: "6B7280",
			font: "Arial"
		})]
	...
	const includeBreakup = [
		"offer",
		"appointment",
		"increment",
		"salary_certificate"
	].includes(template.key);
	let breakupTable = null;
	if (includeBreakup) {
		const p = computePayroll({
			company,
			employee,
		...
		const rows = [
			...
			[
				"Special Allowance",
				p.earnings.special,
				p.earnings.special * 12
			],
			[
				"Medical",
				p.earnings.medical,
				p.earnings.medical * 12
			],
			[
				"Conveyance",
				p.earnings.conveyance,
				p.earnings.conveyance * 12
			],
			[
				"Washing",
				p.earnings.washing,
				p.earnings.washing * 12
			],
			[
				"Other Allowance",
				p.earnings.other,
	...
	return {
		...
		filename: `${template.title.replace(/\s+/g, "_")}_${employee.empCode}.docx`
	};
}
async function downloadLetter(company, employee, template, format, assets) {
	if (format === "pdf") {
...
async function bulkZipLetters(company, employees, template, format, assets) {
...
function buildGenericTemplate(code, title, employee, extra) {
...
function generateAssetHandoverPDF(company, employee, asset, kind, assets) {
	return generateLetterPDF(company, employee, {
		key: "custom",
		title: kind === "handover" ? "Asset Allocation & Handover" : "Asset Return Acknowledgement",
		category: "Onboarding",
		description: "Asset movement",
		body: `This is to record that the asset described below has been ${kind === "handover" ? "issued to" : "returned by"} the employee named herein.
...
//#endregion
export { downloadLetter as a, generateLetterPDF as c, bulkZipLetters as i, renderTemplate as l, buildGenericTemplate as n, generateAssetHandoverPDF as o, buildVars as r, generateLetterDOCX as s, DEFAULT_TEMPLATES as t };
