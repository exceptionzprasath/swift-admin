...
import commingSoonAnimationRaw from "@/assets/commingsoon.json";

const Lottie = (LottieRaw as any)?.default || LottieRaw;
const commingSoonAnimation = (commingSoonAnimationRaw as any)?.default || commingSoonAnimationRaw;
...
export function ComingSoonPage({
  ...
  return (
    <div className="relative min-h-[calc(100vh-140px)] w-full flex items-center justify-center p-4 sm:p-8 overflow-hidden">
      ...
      {/* Centered Frosted Card with Lottie Animation */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ duration: 0.35, ease: "easeOut" }}
        className="relative z-10 max-w-lg w-full rounded-3xl border border-border/80 bg-card/85 backdrop-blur-xl p-6 sm:p-10 text-center shadow-2xl space-y-5"
      >
        {/* Lottie Animation Display */}
        <div className="mx-auto w-full max-w-[260px] sm:max-w-[320px] flex items-center justify-center pointer-events-none">
          <Lottie
        ...
        <div className="flex items-center justify-center">
          <Badge variant="outline" className="text-xs font-semibold text-muted-foreground border-border px-3 py-1">
            <Clock className="h-3.5 w-3.5 mr-1.5 inline" /> {category}
          </Badge>
        </div>
        ...
        <div className="pt-2">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-muted/50 border border-border/60 text-xs font-medium text-muted-foreground">
