...
3. This agreement has been acknowledged and executed via the CreatonsHR Employee Portal.${extra ? `\n\nAdditional Terms:\n${extra}` : ""}
...
function pdfHeader(doc: jsPDF, company: Company, title: string, logoDataUrl?: string) {
  ...
  doc.setFontSize(15);
  doc.text((company.name || "CreatonsHR").toUpperCase(), textLeft, 13);
...
function pdfFooter(doc: jsPDF, refId: string) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.text(
      `Generated by SWIFT AI · CreatonsHR · ${new Date().toLocaleString("en-IN")} · Ref: ${refId}`,
      ML, 289,
    );
...
export function generateLetterPDF(
  ...
  const refId = `CHR-${docKey}-${employee.empCode}-${Date.now().toString(36).toUpperCase()}`;
  ...
  doc.text(`Ref: CHR/${docKey}/${employee.empCode}`, RIGHT, 40, { align: "right" });
  ...
  return new Header({
    children: [
      new Paragraph({
        ...
        children: [
          new TextRun({ text: "CreatonsHR", bold: true, size: 40, color: BRAND_HEX, font: "Arial" }),
  ...
  return new Footer({
    children: [
      new Paragraph({
        ...
        children: [
          new TextRun({ text: "Generated by SWIFT AI · CreatonsHR · ", size: 14, color: "9CA3AF", font: "Arial" }),
  ...
  const meta: Paragraph[] = [
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: `\t\tRef: CHR/${template.key.toUpperCase()}/${employee.empCode}`, size: 20, color: "6B7280", font: "Arial" }),
      ],
    }),
    new Paragraph({
